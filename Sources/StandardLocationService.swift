            ...
            if self.locMgrForLocation.location != nil{
                ...
                return Observable.create{
                    ...
                    return Disposables.create{
                        ownerService.locatedObservers.remove(at: ownerService.locatedObservers.index{$0.id == id}!)
    ...
    var isPaused:Observable<Bool>{
        get{
            return Observable.create {
                ...
                return Disposables.create{
                    ownerService.isPausedObservers.remove(at: ownerService.isPausedObservers.index{$0.id == id}!)
                    ownerService = nil
                }
            }
        }
    ...
    var deferredUpdateFinished:Observable<NSError?>{
        get{
            return Observable.create {
                ...
                return Disposables.create{
                    ownerService.deferredUpdateErrorObservers.remove(at: ownerService.deferredUpdateErrorObservers.index{$0.id == id}!)
                    ownerService = nil
                }
            }
        }
    ...
    var locating:Observable<[CLLocation]> {
        get{
            return Observable.create {
                ...
                return Disposables.create{
                    ownerService.locatingObservers.remove(at: ownerService.locatingObservers.index{$0.id == id}!)
        ...
        locMgrForLocation = bridgeClass.init()
            ...
            if #available(watchOS 3.0, *){
                ...
                locMgrForLocating.didFailWithError = {
                    [weak self]
                    mgr, err in
                    if err.domain == "kCLErrorDomain" && CLError.Code.locationUnknown.rawValue == err.code{
